...
def train_model(model, train_loader, criterion, hyperparameters):
    ...
    utils.plot_loss_accuracy_curves(
        ...
        figSaveDir=f"../figs/convnext_experiments/"
        )
...
def run_convnext_experiment(train_loader, test_loader, hyperparameters, results_list):
    ...
    """
    saveDirResults = f"../data/results/convnext_experiments/{hyperparameters['model_name']}_pretrained{hyperparameters['pretrained_model']}_finetuning{hyperparameters['finetuning']}_optim{hyperparameters['optimizer']}.pkl"
    
    if hyperparameters['model_name'] == 'ConvNeXt-Tiny':
        model = convnext_tiny(weights='DEFAULT' if hyperparameters['pretrained_model'] else None)
    elif hyperparameters['model_name'] == 'ConvNeXt-Base':
        model = convnext_base(weights='DEFAULT' if hyperparameters['pretrained_model'] else None)
    model = model.to(device)
    ...
    with open(saveDirResults, "wb") as f:
        pickle.dump({"train":train_numerical_results, "test":test_numerical_results}, f)
...
logger = utils.setup_logger('../logs/convnext_models_logs.txt')
    ...
    for pretrained_flag in pretrained:
        for finetune_flag in whether_to_finetune:
            for optim_name in optim_names:
                ...
                model, results_list = run_convnext_experiment(train_loader, test_loader, hyperparameters, results_list)
...
with open("../data/results/convnext_experiments/all_results.pkl", "wb") as f:
